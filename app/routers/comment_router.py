...
def comment_post(post_id: int, comment: CommentCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if not user:
    ...
    db_comment = Comment(postid=post_id, userid=user.id, content=comment.content)
    db.add(db_comment)
    db.commit()
